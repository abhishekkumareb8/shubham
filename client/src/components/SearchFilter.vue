    ...
    <b-row>
      <b-col class="mt-3 mb-3">
        <b-input-group>
          ...
          <b-form-input id="search" class="filter-dropdown" v-on:keyup="onSearchKeyUp" v-model="searchText" placeholder="Search users" @focus="$event.target.select()" trim></b-form-input>
        </b-input-group>
      </b-col>
      <b-col class="mt-3 mb-3" cols="12" md="auto"><b-form-select v-model="filter" :options="filterOptions"></b-form-select></b-col>
      <b-col class="mt-3 mb-3" col lg="1"><b-button variant="primary" class="filter-btn" @click="onChange()">Filter</b-button></b-col>
    </b-row>
  </div>
</template>
